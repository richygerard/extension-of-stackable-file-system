...
/* switching to encryption/decryption */
//#define WRAPFS_CRYPTO 1
...
/* useful for tracking the code */
#define TRACK printk(KERN_INFO "TRACK:%s:%s:%d\n", __FILE__, __func__,\
 __LINE__)
#define TRACK_VALUE(value) printk(KERN_CRIT "TRACK_VALUE:%s:%s:%d:Value:%d\n",\
 __FILE__, __func__, __LINE__, value)
#define TRACK_STRING(string) printk(KERN_CRIT "TRACK_STRING:%s:%s:%d:String:%s\
...
/* defined for address space ops for mmap option */
